...
class LocalsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create local" do
    assert_difference("Local.count") do
      post locals_url, params: { local: { address: @local.address, capacity: @local.capacity, description: @local.description, name: @local.name } }
    end
  ...
  test "should update local" do
    patch local_url(@local), params: { local: { address: @local.address, capacity: @local.capacity, description: @local.description, name: @local.name } }
